...
module.exports = class baseController{
	...
	static storeCoordinates(req, res){
	  ...
	  static myinfo(req, res){
		let myposts, myganged = 0, mygang = 0; 
		Promise.all([
			$u.Discover.find({
	...
	static updateEmail(req, res){
		return $u.User.updateEmail().then(users => {
			res.json(users);
		}, error => {
			res.json(error);
			console.log(error)
	...
	static generateAndReplaceWhiteSpacesWithCharacter(name, numberMax, character){
  ...
  static get staffLoginSchema(){
	...
	static logoutAcc(req, res){
	  baseController.destroy(req.body.id, {"token": null})
	  .then(user =>  {
			  res.json(Object.assign(user, {"message": true}))
		  }, err => {
	      	res.json(Object.assign(err,  {"message": false}))
	    }).catch(err => res.json(Object.assign(err.toString(), {"message": false})))
  ...
  static login(req, res){
    ...
    .then(([login, user]) => {
		...
		console.log(user.id);
		baseController.attachToken(user, login)
		.then(user =>  {
			  res.json(Object.assign(user, {token : baseController.prepToken(user.id)}, {"message": true}))
		  }, err => {
	      	res.json(Object.assign(err, {token : baseController.prepToken(user.id)}, {"message": true}))
	    }).catch(err => res.json(Object.assign(err.toString(), {token : baseController.prepToken(user.id)}, {"message": true})))
  ...
  static signup(req, res){
  	...
  	$u.User.signup(Object.assign(req.body, {
		...
		try{
		   request.get("http://142.93.207.91//tdxmailer/signup.php?"+
		   "email="+user.email+ 
		   "&id="+user.id+
		   "&fullname="+user.firstname + ' ' + user.lastname+
		   "&token="+user.status +
		   "&from="+apptype, function(error, response, body){	   
				if(error)
					console.log(error);
				if(response)
  ...
  static getreferee(req, res){ 
    $u.User.getrefere({"userId" : req.params.referer})      
      .then(referee => {
        res.status(200).json(Object.assign({"param": req.param.referer}, referee))
    }, err => {
